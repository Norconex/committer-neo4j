 ...
 * @since 1.3.0
 */
public class PathTagger extends AbstractDocumentTagger {
    ...
    private void breakSegments(
        ...
        for (String value : metadata.getStrings(details.fromField)) {
            ...
            StringTokenizer st = new StringTokenizer( value, details.fromSeparator);
            StringBuilder b = new StringBuilder();
          
            while (st.hasMoreTokens()){
            	String seg = st.nextToken();
            	
                   b.append(seg).append(sep);
            ...
            /*for (int i = 0; i < segs.length -1; i++) {
            	if (segs[i].equals(details.fromSeparator)) {
                    b.append(sep);
                } else {
                    b.append(segs[i]);
                }
    ...
    @Override
    public boolean equals(final Object other) {
        if (!(other instanceof PathTagger)) {
            return false;
        }
        PathTagger castOther = (PathTagger) other;
